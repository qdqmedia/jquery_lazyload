...
* Manage the jQuery dependency internally tring to load from a sibling npm package and fallback on global variable.
* Export the main lazyload function as node package if we are in a node enviroment like browserify.
